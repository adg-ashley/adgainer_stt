      ...
      <q-tr :props="props">
        ...
        <template v-for="col in props.cols.filter(r => r.name !== 'action')">
          ...
          <q-th  key="action" :props="props">
            <q-btn size="sm" v-if="selected.length > 0" color="secondary" outline icon="restore"  @click="restoreRows" label="Restore" dense/>
            ...
            <q-td key="action" style="width: 30%;" align="right">
                ...
                <div>
                  <q-btn-dropdown 
                      ...
                      <!-- dropdown content -->
                      <q-list dense link>
                        <q-item @click.native="restore(props.row)">
                          <q-item-main>
                            <q-item-tile label>Restore</q-item-tile>
                          </q-item-main>
                        </q-item>
  ...
  methods: {
    ...
    deleteRow (row) {
      this.$q.dialog({ 
        ...
        this.delete({url: `/stt/records/permanent/${row.rid}`, params: {
            rid: row.rid
          },
          method: 'DELETE'})
          .then((data)=> {
            ...
            this.$q.notify({ type: 'positive', message: `Deleted record '${row.name}'.`, position: 'bottom-left'})
          })
      })
    },
    openModal (record) {
    ...
    deleteRows () {
       ...
       this.$q.dialog({ 
        ...
        message: `Are you sure you want to permanently delete '${rows.length}' ${ rows.length > 1 ? 'files' : 'file' }? `,
        ok: 'Delete',
        cancel: 'Cancel'
      }).then(() => {
        var c = 1;
        const iterateRows = (rows, cb) => {
          rows.map((r) => {
            this.delete({url: `/stt/records/permanent/${r.rid}`, method: 'DELETE'})
              .then((data)=> {
              //c = c + 1
    ...
    restore(row) {
      this.$q.dialog({ 
        ...
        this.update({url: `/stt/records/restore/${row.rid}`, params: {
            rid: row.rid
          },
          method: 'POST'})
          .then((data)=> {
            this.rowCount = this.rowCount - 1
            this.$q.notify({ type: 'positive', message: `Record '${row.name}' Restored.`, position: 'bottom-left'})
          })
      })
    },
    restoreRows() {
      ...
      message: `Are you sure you want to restore '${rows.length}' ${ rows.length > 1 ? 'files' : 'file' }? `,
      ok: 'Restore',
      cancel: 'Cancel'
      }).then(() => {
        var c = 1;
        const iterateRows = (rows, cb) => {
          rows.map((r) => {
            this.update({url: `/stt/records/restore/${r.rid}`, method: 'POST'})
              .then((data)=> {
              this.rowCount = this.rowCount - 1
              cb(c++)
            })
        ...
        iterateRows(rows, (i) => {    
          if (rows.length === i) {
            this.$q.notify( { type: 'positive', position: 'bottom-left', message: `Restored '${rows.length}' ${ rows.length > 1 ? 'files' : 'file' }.`})
          }
        })
      })
    }
